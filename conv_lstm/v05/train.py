...
data = ConvLSTM2DDataset()
test_data = ValiDataset(data)
...
criterion = nn.L1Loss()
optimizer = torch.optim.Adam(model.parameters(), lr=0.0008, betas=(0.94, 0.94), weight_decay=1.12e-5)
    ...
    for epoch in range(num_epochs):
        ...
        with tqdm(train_loader, leave=False) as pbar_inner:
            for i, (images, labels) in enumerate(pbar_inner):
                ...
                # Forward pass
                outputs = model(images).permute(permutation)
        ...
        with torch.no_grad():
            ...
            for i, (images, labels) in enumerate(test_loader):
                ...
                if i == 0 and epoch % 5 == 0:
                    input_image = images[0].cpu().numpy()[0]
                    target_image = labels[0].cpu().numpy()[0]
