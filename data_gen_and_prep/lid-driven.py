...
def prep_data_for_plotting(u, v, p):
    ...
    p = np.rot90(p, 1)[1:-1, 1:-1]
    return data_interpolated_u, data_interpolated_v, p
...
if __name__ == "__main__":
    ...
    plot = False
    paths = [f"src/data/{Re}" for Re in REs]
    for i, Re in enumerate(REs):
        ...
        with tqdm.tqdm(total=int(t_end/dt)) as pbar:
            while t < t_end:
                ...
                dt = select_dt_according_to_stability_condition(Re, dx, dy, tau, u, v, imax, jmax)
                plot_freq = 0.05
                if (plot and t % plot_freq < dt) or t == dt:
                    data_interpolated_u, data_interpolated_v, p = prep_data_for_plotting(u, v, p)
                    
                    # save the data with PIL
                    current_t = f'{t:.2f}'
                    plt.imsave(f'{path}/u_{current_t}.png', data_interpolated_u, cmap='viridis')
                    plt.imsave(f'{path}/v_{current_t}.png', data_interpolated_v, cmap='viridis')
                    plt.imsave(f'{path}/p_{current_t}.png', p, cmap='viridis')