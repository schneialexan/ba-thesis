...
def imageOut(filename, _outputs, _targets, saveTargets=False, normalize=False, saveMontage=True):
    ...
    for i in range(3):
        ...
        if saveMontage:
            ...
            axes[0, i].imshow(targets[i], cmap='viridis')
            BW_im.paste(im, ( (s+10)*i, s*0))           
            im = Image.fromarray(outputs[i] * 256.)
            axes[1, i].imshow(outputs[i], cmap='viridis')
            ...
            if i==0:
                ...
                prefix = "Velocity Y"
            # first target
            axes[0, i].set_title('Target: ' + prefix)
            # then output
            axes[1, i].set_title('Output: ' + prefix)
            # then error (target - output)
            axes[2, i].set_title('Error (Target - Output)')
    plt.colorbar(cm.ScalarMappable(cmap='gray'), ax=axes[2:])
...
def fullImageOut(filename, _inputs, _outputs, _targets, saveTargets=False, normalize=False, saveMontage=True):
    ...
    for i in range(3): 
        ...
        if saveMontage:
            ...
            axes[0, i].set_title('Input: ' + prefix)
